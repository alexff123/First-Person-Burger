...
> 排障：每次运行都会覆写 `game/launcher.log`，里面记录了走到哪一步。

**改这个 .bat 时必须守住的三条**（都是踩过的坑，详见 `CODE_REVIEW.md`）：

1. **内容必须纯 ASCII**。含中文会让 cmd 丢失批处理文件指针、`::` 前缀被吃掉，
   注释行会被当成命令执行。校验：文件里 >127 的字节数必须为 0。
2. **探测必须 `--noproxy "*"` + `-f`**。系统代理会把 `localhost` 请求拦走并回 502，
   而 curl 认为"HTTP 事务完成"就算成功 → 返回 0 → 误判成"服务器已在运行" → 打开死链。
3. **探测 URL 用 `localhost`，不能只用 `127.0.0.1`**。Windows 上 Vite 默认只绑 IPv6 的
   `::1`，探 `127.0.0.1` 会假阴性。当前实现先探 `localhost`，失败再探 `127.0.0.1` 兜底。
